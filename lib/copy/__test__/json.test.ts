import json from "../src/json"

describe("nilEmpty", () => {
    it("target is undefined", () => {
        expect(json(undefined)).toBe(undefined)
    })
    it("target is null", () => {
        expect(json(null)).toBe(null)
    })
    it("target is value", () => {
        expect(json("string")).toBe("string")
        expect(json(1)).toBe(1)
        expect(json(0)).toBe(0)
        expect(json(0x36)).toBe(0x36)
        expect(json(true)).toBe(true)
        expect(json(false)).toBe(false)
    })
    it("target is array", () => {
        const origin = [1, 2, 3, 4, 5]
        const target = json(origin)
        expect(Array.isArray(target)).toBe(true)
        expect(origin !== target).toBe(true)
        expect(origin.length === target.length).toBe(true)
        for (let i = 0; i < origin.length; i++) {
            expect(origin[i] === target[i]).toBe(true)
        }
    })
    it("target is object array", () => {
        const origin = [{ a: "a" }, { b: "b" }, { c: "c" }]
        const target = json(origin)
        expect(Array.isArray(target)).toBe(true)
        expect(origin !== target).toBe(true)
        expect(origin.length === target.length).toBe(true)
        for (let i = 0; i < origin.length; i++) {
            expect(origin[i] !== target[i]).toBe(true)
        }
    })
    it("target is object", () => {
        const origin = { a: "a", b: "b", c: "c" } as Record<string, any>
        const target = json(origin)
        expect(origin !== target).toBe(true)
        const originKeys = Object.keys(origin)
        const targetKeys = Object.keys(target)
        expect(originKeys.length === targetKeys.length).toBe(true)
        for (let i = 0; i < originKeys.length; i++) {
            expect(originKeys[i] === targetKeys[i]).toBe(true)
            expect(origin[originKeys[i]] === target[targetKeys[i]]).toBe(true)
        }
    })
    it("target is nested object", () => {
        const origin = { a: { a: "a" }, b: { b: "b" }, c: { c: "c" } } as Record<string, any>
        const target = json(origin)
        expect(origin !== target).toBe(true)
        const originKeys = Object.keys(origin)
        const targetKeys = Object.keys(target)
        expect(originKeys.length === targetKeys.length).toBe(true)
        for (let i = 0; i < originKeys.length; i++) {
            expect(originKeys[i] === targetKeys[i]).toBe(true)
            expect(origin[originKeys[i]] !== target[targetKeys[i]]).toBe(true)
        }
    })
})